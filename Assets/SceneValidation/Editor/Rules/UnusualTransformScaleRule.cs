...
public sealed class UnusualTransformScaleRule : ValidationRule {
	...
	private void ValidateGameObject(GameObject gameObject, ValidationResultCollection results) {
		...
		if (scale.x > 10f || scale.y > 10f || scale.z > 10f) {
			results.AddResult(Severity, Id, Name, $"Unusual scale detected.", $"GameObject <i><b>{gameObject.name}</b></i> has an unusual scale: {scale}.", gameObject);
		}

		if (scale.x < 0 || scale.y < 0 || scale.z < 0) {
			results.AddResult(Severity, Id, Name, "Unusual scale detected.", $"GameObject <i><b>{gameObject.name}</b></i> has negative scale: {scale}.", gameObject);
		}
